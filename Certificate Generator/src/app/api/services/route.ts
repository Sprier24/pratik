...
export async function POST(request: Request) {
  try {
    ...
    const requiredFields = [
      "id", "serviceId", "customerName", "customerLocation", "contactPerson",
      "contactNumber", "serviceEngineer", "date", "place", "placeOptions",
      "natureOfJob", "reportNo", "makeModelNumberoftheInstrumentQuantity",
      "serialNumberoftheInstrumentCalibratedOK", "serialNumberoftheFaultyNonWorkingInstruments",
      "engineerReport", "customerReport", "engineerRemarks", "engineerName"
    ];
    ...
    await client.execute({
      ...
      args: [
        ...
        body.engineerReport,
        body.customerReport,
        JSON.stringify(body.engineerRemarks), // store as JSON
        body.engineerName,
        body.status ?? "checked"
      ],
    });
