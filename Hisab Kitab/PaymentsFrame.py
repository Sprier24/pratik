...
class PaymentsFrame(tk.Frame):
    def __init__(self, root):
        ...
        self.load_records()
        self.editing_record_id = None  # Initialize for editing
    ...
    def setup_filter(self):
        ...
        # Set width for the "Clear Filter" button
    ...
    def setup_search(self):
        ...
        # Set width for the "Search" button
    ...
    def load_records(self):
        ...
        # Updated query to include the `id` column
        select_query = "SELECT id, name, description, amount_due, due_date, paid_unpaid FROM payments"
        self.cursor.execute(select_query)
        records = self.cursor.fetchall()

        for record in records:
            # Insert into table without displaying `id` by using the tuple unpacking
            self.table.insert("", tk.END, values=record[1:], iid=record[0])  # `iid` here is the `id`
    ...
    def submit(self):
        ...
        if self.editing_record_id:  # If editing an existing record
            if messagebox.askyesno("Confirm Update", "Are you sure you want to update this record?"):
                ...
                self.editing_record_id = None  # Reset after editing
        else:
        ...
        self.load_records()  # Refresh the table
        self.clear_entries()
    ...
    def clear_entries(self):
        ...
        self.due_date_entry.set_date(datetime.datetime.now())  # Reset to today
        self.paid_unpaid_var.set("Unpaid")

    def apply_filter(self, value):
        self.table.delete(*self.table.get_children())  # Clear the table before loading new data
        ...
        if value == "All":
            ...
            self.cursor.execute(select_query, (value,))  # Filter by "Paid" or "Unpaid"

        records = self.cursor.fetchall()

        for record in records:
            self.table.insert("", tk.END, values=record[1:], iid=record[0])  # Insert without displaying `id`
    ...
    def edit(self):
        ...
        # Set the form fields with the selected record's values
        self.name_entry.delete(0, tk.END)
        self.name_entry.insert(0, values[0])  # Name
        self.description_entry.delete(0, tk.END)
        self.description_entry.insert(0, values[1])  # Description
        self.amount_due_entry.delete(0, tk.END)
        self.amount_due_entry.insert(0, values[2])  # Amount Due
        self.due_date_entry.set_date(datetime.datetime.strptime(values[3], '%Y-%m-%d'))  # Due Date
        self.paid_unpaid_var.set(values[4])  # Paid/Unpaid

        # Set editing record id to the `iid`
        self.editing_record_id = selected_item  # This is the `id` directly
