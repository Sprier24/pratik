...
class InvoiceFrame(tk.Frame):
    ...
    def update_totals(self):
        ...
        for row in self.table.get_children():
            ...
            if values:
                total_without_gst = float(values[11]) 
                grand_total_with_gst = float(values[12])
    ...
    def generate_pdf(self):
     ...
     for item in self.table.get_children(): 
         ...
         name_matches = search_name in str(values[1]).lower()
         date_matches = search_date == '' or values[3] == search_date
         if name_matches and date_matches:
             if values[1] not in grouped_rows:
                 grouped_rows[values[1]] = []
