...
const BillPage = () => {
  ...
  useEffect(() => {
    ...
    if (params.serviceData) {
      try {
        ...
        // Parse services from serviceData
        if (serviceData.services) {
          try {
            const servicesArray = typeof serviceData.services === 'string' 
              ? JSON.parse(serviceData.services) 
              : serviceData.services;
            
            setServicesList(servicesArray.map((s: any) => ({ 
              name: s.name || s.serviceType || '', 
              charge: s.charge || s.serviceCharge || '' 
            })));
          } catch (e) {
            console.error('Error parsing services:', e);
            // If services parsing fails, create from serviceType
            if (serviceData.serviceType) {
              const servicesArray = serviceData.serviceType.split(',').map((s: string) => s.trim());
              setServicesList(servicesArray.map(s => ({ 
                name: s, 
                charge: '' 
              })));
            }
          ...
          // Create services list from serviceType
          const servicesArray = serviceData.serviceType.split(',').map((s: string) => s.trim());
          setServicesList(servicesArray.map(s => ({ 
            name: s, 
            charge: serviceData.serviceCharge || '' 
          })));
        }
  ...
  // Add service charge calculation effect
  useEffect(() => {
    const total = servicesList.reduce((sum, service) => {
      return sum + (parseFloat(service.charge) || 0);
    }, 0);
    
    // Update service charge in form
    setForm(prev => ({ ...prev, serviceCharge: total.toString() }));
  }, [servicesList]);

  const handleServiceChargeChange = (index: number, value: string) => {
    const updatedServices = [...servicesList];
    updatedServices[index].charge = value;
    setServicesList(updatedServices);
  };

  const addNewService = () => {
    setServicesList(prev => [...prev, { name: '', charge: '' }]);
  };
  ...
  const validateForm = () => {
    ...
    if (servicesList.some(service => !service.name.trim())) {
      Alert.alert('Error', 'Service name is required for all services');
      return false;
    }
    if (servicesList.some(service => !service.charge.trim() || isNaN(parseFloat(service.charge)))) {
      Alert.alert('Error', 'Valid service charges are required for all services');
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    const billData = {
      ...
      notes: notes.trim() || '',
      billNumber,
      serviceType: servicesList.map(s => s.name).join(', '),
  ...
  const generateBillHtml = (bill: Bill) => {
    const servicesHtml = bill.services ? bill.services.map(service => `
    ...
    return `
       <html>
              ...
              <body>
                ...
                <div class="section">
                  <div class="section-title">Engineer Details</div>
                  <div class="row">
          ...
          <ScrollView contentContainerStyle={[styles.scrollContainer, { paddingBottom: 150 }]} keyboardShouldPersistTaps="handled">
            <View style={styles.formContainer}>
              ...
              <View style={styles.servicesContainer}>
                <Text style={styles.servicesTitle}>Services & Charges</Text>
                  ...
                  <View key={index} style={styles.serviceItem}>
                    <View style={styles.serviceInputRow}>
                      <TextInput
                        ...
                        value={service.name}
                        onChangeText={(text) => handleServiceNameChange(index, text)}
                      />
                      <TextInput
                        style={[styles.input, styles.serviceChargeInput]}
                        placeholder="Amount"
                        value={service.charge}
                        onChangeText={(text) => handleServiceChargeChange(index, text)}
                ...
                >
                  <Feather name="plus" size={20} color="#5E72E4" />
              ...
              <View style={styles.paymentSummary}>
                <View style={[styles.summaryRow]}>
                  ...
                  <Text style={styles.summaryValue}>₹{form.serviceCharge}</Text>
                </View>
                <View style={[styles.summaryRow]}>
                  <Text style={styles.summaryLabel}>GST ({gstPercentage}%) :</Text>
                  <Text style={styles.summaryValue}>₹{(parseFloat(form.serviceCharge) * parseFloat(gstPercentage) / 100).toFixed(2)}</Text>
                </View>
                <View style={[styles.summaryRow, styles.totalRow]}>
                  <Text style={[styles.summaryLabel, styles.totalLabel]}>Total Amount :</Text>
                  <Text style={[styles.summaryValue, styles.totalValue]}>₹{calculateTotal()}</Text>
                </View>
              </View>
                  ...
                  <View style={styles.detailSection}>
                    ...
                    {selectedBill.services && selectedBill.services.map((service, index) => (
                      <View key={index} style={styles.detailRow}>
                        ...
                        <Text style={styles.detailValue}>₹{service.charge}</Text>
                      </View>
                    ))}
                    <View style={styles.detailRow}>
